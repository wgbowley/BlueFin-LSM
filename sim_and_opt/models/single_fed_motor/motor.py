...
class Tubular(LinearBase):
    ...
    def add_stator(self) -> None:
        ...
        # height = self.total_number_poles * self.pole_pitch
        # radius = self.pole_inner_radius

        # origin = (0, -1 / 2 * height + self.pole_axial_length)
    ...
    def compute_geometry(self) -> None:
        ...
        # Bigger or equal to pole_pitch (NEED TO REGENERATE PITCHES AFTER THIS THOUGH)
        # if self.pole_pitch < self.pole_axial_length:
        #     min = self.pole_axial_length * self.number_poles
        #     self.slot_axial_length = (
        #         -(self.slot_axial_spacing * (self.number_slots - 1) - min) / self.number_slots
        #     )
        #     msg = f"Pitch > Axial length; set to {self.slot_axial_length:.3f} mm"
        #     logging.warning(msg)

...
Description:
    Custom double field linear motor for
    high performance 3d printers.
    - No magnetic materials
    - Copper coils for stator
    - Copper coils for armuture
...
class Tubular(LinearBase):
    ...
    def __init__(self, parameter_file) -> None:
        ...
        self.group_armature: int = 1
        self.group_stator: int = 2
    ...
    def add_armature(self) -> None:
        ...
        armature_origins = origin_points(
            self.number_slots,
            0,
            self.armature_pitch,
            x_offset=self.armature_inner_radius,
        )

        # Calculates turns within the slot cross section
        number_turns = estimate_turns(
            self.armature_thickness,
            self.armature_axial_length,
            self.armature_wire_diameter,
            self.fill_factor
        )

        for index, origin in enumerate(armature_origins):
    ...
    def add_stator(self) -> None:
        ...
        """
        femm.mi_addcircprop("phase_d", 0, 1)
        femm.mi_setcurrent("phase_d", self.static_currents)

        # Calculates turns within the slot cross section
        number_turns = estimate_turns(
            self.stator_thickness,
            self.stator_axial_length,
            self.stator_wire_diameter,
            self.fill_factor
        )

        # Generate pole origin points, shifted axially by extra pairs
        stator_origins = origin_points(
            self.total_number_poles,
            0,
            self.stator_pitch,
            y_offset=-2 * (self.extra_pairs * self.stator_pitch),
            x_offset=self.stator_inner_radius
        )

        for index, origin in enumerate(stator_origins):
            turns = number_turns if index % 2 == 0 else -number_turns
        ...
        height = self.total_number_poles * self.stator_pitch
        radius = self.stator_inner_radius

        origin = (0, -1 / 2 * height + self.stator_axial_length)
    ...
    def add_boundary(self) -> None:
        ...
        origin = (0, self.stator_axial_length * self.number_poles * 0.5)

        # Radial extent based on stator poles and pitch
        stator_radius = 0.5 * (self.total_number_poles) * self.stator_pitch

        # Radial extent including armature and slot height
        armature_radius = self.armature_outer_radius
    ...
    def compute_geometry(self) -> None:
        ...
        self.armature_pitch = self.armature_axial_length + self.armature_axial_spacing

        # The pattern length includes air gaps not physically present.
        # Subtract to get the actual armature circumference.
        pattern_length = self.armature_pitch * self.number_slots
        air_gaps = 1 * self.armature_axial_spacing

        self._circumference = pattern_length - air_gaps

        # Calculates pole start-to-start axial distance
        self.stator_pitch = self.circumference / self.number_poles

        # Ensures that the program doesn't crash from overlapping poles/slots
        if self.stator_pitch != self.stator_axial_length:
            self.stator_axial_length = self.stator_pitch
            msg = f"Pitch > Axial length; set to {self.stator_pitch:.3f} mm"
            logging.warning(msg)
    ...
    def add_materials(self) -> None:
        ...
        add_femm_material(materials, self.armature_material)
        add_femm_material(materials, self.stator_material)
    ...
    def _unpack(self, parameter_file: str) -> None:
        ...
        required_sections = ["model", "armature", "stator", "output"]
        ...
        self.static_currents = require("static_current", model)

        self.fill_factor = require("fill_factor", model)
        self.boundary_material = require("boundary_material", model)

        # Assign slot parameters
        self.armature_inner_radius = require("inner_radius", armature)
        self.armature_outer_radius = require("outer_radius", armature)
        self.armature_axial_length = require("axial_length", armature)
        self.armature_axial_spacing = require("axial_spacing", armature)

        self.armature_material = require("material", armature)
        self.armature_wire_diameter = require("wire_diameter", armature)

        # Assign pole parameters
        self.stator_inner_radius = require("inner_radius", stator)
        self.stator_outer_radius = require("outer_radius", stator)
        self.stator_axial_length = require("axial_length", stator)
        self.stator_material = require("material", stator)
        self.stator_wire_diameter = require("wire_diameter", stator)

        # Calculates radial thickness for slot, tube & poles
        self.armature_thickness = self.armature_outer_radius - self.armature_inner_radius
        self.stator_thickness = self.stator_outer_radius - self.stator_inner_radius
